...
const iotRecommendationSchema = {
    ...
    required: ['iot_recommendations'],
};


export const analyzeEmotion = async (text: string): Promise<Omit<EmotionResult, 'iot_recommendations'>> => {
  const systemInstruction = `당신은 텍스트에서 인간의 감정을 분석하는 전문가 AI입니다. 제공된 텍스트를 기반으로 주요 감정, 신뢰도 점수, 분석 근거, 감정을 나타내는 이모지를 식별합니다. 모든 응답은 한국어로 해주세요.`;
  console.log("Emotion Analysis System Instruction:", systemInstruction);
...
export const recommendIotDevices = async (emotion: string, deviceIds: string[]): Promise<{ iot_recommendations: EmotionResult['iot_recommendations'] }> => {
    ...
    try {
        ...
        console.error("Error recommending IoT devices:", error);
        if (error instanceof Error) {
            throw new Error(`IoT 기기 추천 중 오류 발생: ${error.message}`);
        }
        throw new Error("알 수 없는 오류로 IoT 기기 추천에 실패했습니다.");
    }
};
